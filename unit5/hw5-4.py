...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #
        #  
        # car starts at x = 0, y = r and runs clockwise around
        # track that is 2r in the y and 4r in the x
        # if self.x is between r and 3r we are on straights
        #   then if y > r we are on top, y < r we are on bottom

        left_center = [radius, radius]
        right_center = [3.0*radius, radius]
        if self.x >= radius and self.x < 3.0 * radius:
            if self.y <= radius:
                #bottom straight
                cte = -1.0 * self.y
            else:
            ...
            #left curve
            r = sqrt(pow((self.x - left_center[0]),2) + pow((self.y - left_center[1]),2))
            cte = r - radius
        else:
            #right curve
            r = sqrt(pow((self.x - right_center[0]),2) + pow((self.y - right_center[1]),2))
            cte = r - radius
        return cte
...
def run(params, radius, printflag = False):
    ...
    for i in range(N*2):
        ...
        if printflag:
            print myrobot
            #print "    data.addRow([%s, %s])" % (myrobot.x, myrobot.y)
...
mtpl.figure()
mtpl.hold(True)
mtpl.plot([p[0] for p in carhistory],[p[1] for p in carhistory], label='Path')
mtpl.legend()
mtpl.show()