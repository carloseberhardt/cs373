...
def run(proportional, differential, integral):
    myrobot = robot()
    ...
    for i in range(N):
        ...
        prop_adj = -1.0 * proportional * crosstrack_error
        diff_adj = -1.0 * differential * (crosstrack_error - prev_crosstrack_error)/time
        intg_adj = -1.0 * integral * total_crosstrack_error
        steering = prop_adj + diff_adj + intg_adj
